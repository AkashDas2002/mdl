...
def run(filename):
    ...
    for command in commands:
        ...
        if command['op'] == 'push':
            ...
            translate = make_translate(float(command['args'][0]), float(command['args'][1]), float(command['args'][2]))
            matrix_mult(stack[-1], translate)
            stack[-1] = [x[:] for x in translate]

        elif command['op'] == 'rotate':
            if command['args'][0] == 'x':
                rotate = make_rotX(command['args'][1] * math.pi / 180)
            elif command['args'][0] == 'y':
                rotate = make_rotY(command['args'][1] * math.pi / 180)
            else:
                rotate = make_rotZ(command['args'][1] * math.pi / 180)
            ...
            add_sphere(coords1, float(command['args'][0]), float(command['args'][1]), float(command['args'][2]), float(command['args'][3]), step_3d)
            matrix_mult(stack[-1], coords1)
            if command['constants']:
                draw_polygons(coords1, screen, zbuffer, view, ambient, light, symbols, command['constants'])
            else:
